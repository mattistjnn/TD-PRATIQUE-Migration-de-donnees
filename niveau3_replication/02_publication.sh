...
#TODO
# Creer une publication partielle (seulement certaines tables)
docker exec pg_montpellier psql -U admin -d techcorp_db -c \
  "CREATE PUBLICATION pub_partielle FOR TABLE utilisateurs, formations;"

# Verifier la publication partielle
docker exec pg_montpellier psql -U admin -d techcorp_db -c \
  "SELECT pubname, puballtables FROM pg_publication;"

echo 'OK : Publication partielle creee (utilisateurs + formations uniquement)'
