...
docker exec pg_montpellier psql -U admin -d techcorp_db -c \   "DROP PUBLICATION IF EXISTS pub_techcorp;" 
 
echo 'OK : Publication supprimee sur la source' 
 
# Verification finale 
echo '--- Etat final de la cible ---' docker exec pg_toulouse psql -U admin -d techcorp_db -c \   "SELECT 'utilisateurs', COUNT(*) FROM utilisateurs 
